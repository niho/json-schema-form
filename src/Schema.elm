...
view : State -> Html Msg
view state =
    schemaView [] state.schema state.form


schemaView : Path -> Schema -> Form -> Html Msg
...
objectView : Path -> SubSchema -> Form -> Html Msg
...
fieldView : Path -> SubSchema -> SingleType -> Form -> Html Msg
...
txt : SubSchema -> F.FieldState ValidationError String -> Html Msg
txt schema f =
    field schema f <|
        Input.textInput f
            [ class "form-control"
...
checkbox : SubSchema -> F.FieldState ValidationError Bool -> Html Msg
checkbox schema f =
    fieldset
        [ classList
            [ ( "form-group", True )
...
select : SubSchema -> F.FieldState ValidationError String -> Html Msg
select schema f =
    let
        options =
            case schema.enum of
...
field : SubSchema -> F.FieldState ValidationError String -> Html Msg -> Html Msg
...
error : ErrorString ValidationError -> ErrorValue ValidationError -> Html msg
error func err =
